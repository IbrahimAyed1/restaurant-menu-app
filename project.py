...
from db_test import Base, Restaurant, MenuItem, User
...
def fbconnect():
    ...
    '''
        Due to the formatting for
        the result from the server token exchange we have to
        split the token first on
        commas and select the first
        index which gives us the key : value
        for the server access token
        then we split it on colons
...
@app.route('/restaurant/<int:restaurant_id>/'
           '<int:menu_id>/edit/', methods=['POST', 'GET'])
...
@app.route('/restaurant/<int:restaurant_id>/'
           '<int:menu_id>/delete', methods=['POST', 'GET'])
...
def disconnect():
    if 'provider' in login_session:
        ...
        flash("You have to loggin first")
        return url_for('showRestaurants')
